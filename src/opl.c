...
static doom_boolean music_initialized = false;
...
static doom_boolean opl_stereo_correct = false;

// Load instrument table from GENMIDI lump:

static doom_boolean LoadInstrumentTable(void)
{
    byte *lump;
...
static void LoadOperatorData(int operator, genmidi_op_t *data,
                             doom_boolean max_level, unsigned int *volume)
{
    int level;
    ...
    *volume = level;
    ...
    OPL3_WriteReg(&chip, OPL_REGS_LEVEL + operator, level);
    OPL3_WriteReg(&chip, OPL_REGS_TREMOLO + operator, data->tremolo);
    OPL3_WriteReg(&chip, OPL_REGS_ATTACK + operator, data->attack);
    OPL3_WriteReg(&chip, OPL_REGS_SUSTAIN + operator, data->sustain);
    OPL3_WriteReg(&chip, OPL_REGS_WAVEFORM + operator, data->waveform);
}
                               ...
                               unsigned int instr_voice)
{
    ...
    OPL3_WriteReg(&chip, (OPL_REGS_FEEDBACK + voice->index) | voice->array,
...
static void SetVoiceVolume(opl_voice_t *voice, unsigned int volume)
{
    ...
    if (car_volume != (voice->car_volume & 0x3f))
    {
        ...
        OPL3_WriteReg(&chip, (OPL_REGS_LEVEL + voice->op2) | voice->array,
                          voice->car_volume);
         ...
         && opl_voice->modulator.level != 0x3f)
        {
            ...
            if(mod_volume != voice->mod_volume)
            {
                ...
                OPL3_WriteReg(&chip, (OPL_REGS_LEVEL + voice->op1) | voice->array,
                                  mod_volume |
...
static void VoiceKeyOff(opl_voice_t *voice)
{
    OPL3_WriteReg(&chip, (OPL_REGS_FREQ_2 + voice->index) | voice->array,
                      voice->freq >> 8);
}

static opl_channel_data_t *TrackChannelForEvent(midi_event_t *event)
{
...
static void KeyOffEvent(midi_event_t *event)
{
    opl_channel_data_t *channel;
    int i;
    unsigned int key;
...
static void UpdateVoiceFrequency(opl_voice_t *voice)
{
    ...
    if (voice->freq != freq)
    {
        OPL3_WriteReg(&chip, (OPL_REGS_FREQ_1 + voice->index) | voice->array,
                          freq & 0xff);
        OPL3_WriteReg(&chip, (OPL_REGS_FREQ_2 + voice->index) | voice->array,
                          (freq >> 8) | 0x20);
...
static void KeyOnEvent(midi_event_t *event)
{
    genmidi_instr_t *instrument;
    opl_channel_data_t *channel;
...
static void ProgramChangeEvent(midi_event_t *event)
{
    opl_channel_data_t *channel;
    int instrument;
...
static void SetChannelPan(opl_channel_data_t *channel, int pan)
{
    int reg_pan;
    int i;
...
static void ControllerEvent(midi_event_t *event)
{
    opl_channel_data_t *channel;
...
static void PitchBendEvent(midi_event_t *event)
{
    opl_channel_data_t *channel;
    int i;
...
void OPL_NextEvent(uint32_t event_int) {
    // TODO convert to a midi event.
    midi_event_t event;
    uint8_t data1 = (event_int >> 8) & 0x7f;
    uint8_t data2 = (event_int >> 16) & 0x7f;
    uint8_t channel = event_int & 0xf;
    uint8_t event_type = event_int & 0xf0;
    if (event_type < 0x80 || event_type == 0xf0) {
        return;
    }
    event.event_type = event_type;
    event.data.channel.channel = channel;
    event.data.channel.param1 = data1;
    event.data.channel.param2 = data2;
    ProcessEvent(&event);
}

void OPL_Sample(int16_t *out) {
    OPL3_GenerateResampled(&chip, out);
}
...
doom_boolean I_OPL_InitMusic(void)
{
